...
class LoggerInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (requestHeader) {
      ...
      requestHeaders.addAll(options.headers.map((key, value) {
        if (key == "Authorization") {
          return MapEntry(key, '${(value as String).substring(0, 15)}...');
        }
  ...
  void _printPrettyMap(
    ...
    var tab = tabs;
    final isRoot = tab == initialTab;
    final initialIndent = _indent(tab);
    tab++;
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          debugPrint('║${_indent(tab)} $key: $value${!isLast ? ',' : ''}');
        } else {
          debugPrint('║${_indent(tab)} $key: {');
          _printPrettyMap(value, tabs: tab);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          debugPrint('║${_indent(tab)} $key: ${value.toString()}');
        } else {
          debugPrint('║${_indent(tab)} $key: [');
          _printList(value, tabs: tab);
          debugPrint('║${_indent(tab)} ]${isLast ? '' : ','}');
        }
      } else {
        final msg = value.toString().replaceAll('\n', '');
        final indent = _indent(tab);
        final linWidth = maxWidth - indent.length;
        if (msg.length + indent.length > linWidth) {
          final lines = (msg.length / linWidth).ceil();
          for (var i = 0; i < lines; ++i) {
            debugPrint(
                '║${_indent(tab)} ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}');
          }
        } else {
          debugPrint('║${_indent(tab)} $key: $msg${!isLast ? ',' : ''}');
        }
      }
    });
